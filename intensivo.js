...
function monto_exacto(ft,fd){
    exacto = (parseFloat("0"+ft.t_monto.value,10) <= parseFloat("0"+fd.t_dep.value,10));
    if (!exacto) {
...
function actualizar_total_dep(fc) {
    ...
    with (fc){
        for(i=0;i < m_dep.length;i++){
            tdep1=parseFloat("0"+m_dep[i].value,10);
            tdep+=tdep1;
			//alert('['+i+']'+m_dep[i].value);
        }
        for(i=0;i < m_depH.length - 1;i++){
            tdep1=parseFloat("0"+m_depH[i].value,10);
            tdep+=tdep1;
			//alert('['+i+']'+m_depH[i].value);
        }
		
		(p_exo.value.length == 12) ? tdep+=/*m_exo.value*/parseFloat("0"+m_exo.value,10) : tdep;
...
function actualizarTotales(fp,ft,$update) {
    ...
    ct_uc		= 0;
    ct_monto	= 0;
    v_materia	= parseFloat(ft.valor_materia.value,10); //0.2 Unidades Tributarias
    k =fp.CB.length - 1;
    with(fp) {
       j = 0;
       while(j < k){
...
// cFaltantes : Una lista separada por espacios de los correquisitos que le faltan por inscribir
	...
	//aCorreq = cq.split("_");
	aCorreq = cq.split("-");// Para 2 correquisitos
...
function correquisitoOK(fp) {
    ...
    if (!cOK){
		...
		//OJO Deshabilitado para INTENSIVO 2011
...
function excesoDeCreditosPOZ(lSeccion) {
    ...
    if (matAinsc!=0 ){
        ...
        if (veces != '') {
			...
			if (veces != '') {
				if (intensivo){
					mcausa = "Te excederás de lo estipulado en el reglamento: \n";
					mcausa = mcausa + " Tres asignaturas \n";
					/*mcausa = mcausa + " (b) Hasta diez créditos.\n";*/
					mensLC = " \n";
				}
				else {
				mcausa = "La condición de repitencia de la asignatura \n";
